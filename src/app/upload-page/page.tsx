...
export default function UploadPage() {
  ...
  return (
    ...
    >
      <h1 className="text-4xl sm:text-5xl font-extrabold text-center mb-6">📤 写真アップロード</h1>
      ...
      <label className="flex flex-col items-center bg-gray-800 px-6 py-6 border-2 border-black dark:border-white rounded-2xl cursor-pointer hover:bg-gray-700 transition">
        <span className="text-lg">{files.length ? `${files.length} ファイル選択済み` : "ファイルを選択してください"}</span>
        <input type="file" multiple className="hidden" onChange={handleFileChange} />
      </label>

      {/* アップロード前プレビュー */}
      {previews.length > 0 && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-4 flex flex-wrap gap-4">
          {previews.map((url, i) => (
            <div key={i} className="flex flex-col items-center">
              <p className="text-gray-400 text-sm">プレビュー {i + 1}</p>
              <img src={url} alt={`preview-${i}`} className="max-w-xs rounded-xl shadow-md" />
            </div>
          ))}
        </motion.div>
      )}
      ...
      {success && uploadedUrls.length > 0 && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-4 flex flex-wrap gap-4">
          {uploadedUrls.map((url, i) => (
            <div key={i} className="flex flex-col items-center">
              <p className="text-green-400 font-bold text-sm">アップロード成功 {i + 1}</p>
              <img src={url} alt={`uploaded-${i}`} className="max-w-xs rounded-xl shadow-lg" />
            </div>
          ))}
        </motion.div>
      )}
